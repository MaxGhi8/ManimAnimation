...
class FNO_architecture_1d(ThreeDScene):
    def construct(self):
        ...
        self.play(
            ReplacementTransform(
                Group(tensor_group1, tensor_group2), tensor_group_input
            ),
            run_time=1,
        )
        ...
        label1 = MathTex(
            r"W_{\mathcal{P}} v_0(x_{1}) + b_{\mathcal{P}}", font_size=label_font_big
        )
        label2 = MathTex(
        ...
        for i in range(1, tensor_size[0] + 1):
            ...
            if i == tensor_size[0]:
                ...
                label1_2 = MathTex(
                    r"W_{\mathcal{P}} v_0(x_{2}) + b_{\mathcal{P}}",
        ...
        tensor_group_W = self.createTensor(
            (d_v, n_points, 1), length, spacing, color=BLUE
        )
        ...
        tensor_group_V1 = self.createTensor(
            (d_v, n_points, 1), length, spacing, color=GREEN
        )
        ...
        tensor_group_F_theta = self.createTensor(
            (d_v, k_max, 1), length, spacing, color=RED
        )
        tensor_group_F_theta.scale(0.5)
        tensor_group_F_theta.move_to(2 * RIGHT)
        etichetta5 = MathTex(
            r"\hat{\mathbf{v}}_{2}^{glob}(k)", font_size=etichette_font
        )
        ...
        text9 = MathTex(
            r"\hat{\mathbf{v}}_{2}^{glob}(k) = R_{\Theta, 1}(k) \hat{\mathbf{v}}_{1}(k) \\ \forall k \in \mathbb{Z}_{k_{max}}",
            font_size=text_font,
        )
        text9.to_edge(RIGHT)
        ...
        tensor_group_F_1 = self.createTensor(
            (d_v, n_points, 1), length, spacing, color=RED
        )
        ...
        tensor_group_V = self.createTensor(
            (d_v, n_points, 1), length, spacing, color=BLUE
        )
        ...
        self.play(
            ...
            etichetta8.animate.shift(5.4 * LEFT),
            run_time=1,
        )
        text12 = MathTex(
            r"\text{Apply the} \\ \text{projection operator } \mathcal{Q}",
            font_size=text_font,
        )
        ...
        # create a label above the arrow
        label1 = MathTex(
            r"W_{\mathcal{Q}} v_L(x) + b_{\mathcal{Q}}", font_size=label_font_big
        )
        label2 = MathTex(
