...
class FNO_architecture(ThreeDScene):
    def construct(self):
        ...
        etichetta4 = MathTex(r"\hat{\mathbf{v}}_{1}(\mathbf{k})", font_size=etichette_font)
        ...
        etichetta5 = MathTex(r"\hat{\mathbf{v}}_{2}^{glob}(\mathbf{k})", font_size=etichette_font)
        ...
        self.play(GrowArrow(arrow))
        label = MathTex(r"R_{\Theta, t} \in \mathbb{C}^{d_v \times d_v \times k_{max} \times k_{max}}", font_size=label_font_small)
        label.next_to(arrow, UP, buff=0.2)
        self.play(FadeIn(label), run_time=1) 
        ...
        etichetta3.next_to(tensor_group_V1, UP, buff=0.2)
        self.play(Create(etichetta3), run_time=1)
        self.wait()
        ...
        self.play(tensor_group_V.animate.move_to(4*LEFT), etichetta8.animate.shift(7*LEFT), run_time=1)
        text12 = MathTex(r"\text{Apply the projection operator } \mathcal{Q}", font_size=text_font)
